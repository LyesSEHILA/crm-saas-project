  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8 max-w-7xl mx-auto space-y-10 transition-colors duration-300">
      <header>
        ...
        <p className="text-slate-500 dark:text-slate-400 font-medium">Résumé de votre activité commerciale.</p>
      </header>
        ...
        <div className="bg-slate-900 dark:bg-black p-8 rounded-[2.5rem] text-white shadow-2xl transition-colors">
          ...
          <div className="space-y-4">
            {stats.upcomingTasks.length > 0 ? stats.upcomingTasks.map((task: any) => (
              <div key={task.id} className="p-4 bg-white/5 dark:bg-white/10 rounded-2xl border border-white/10 group hover:bg-white/10 dark:hover:bg-white/20 transition-all">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm italic">Aucune tâche urgente.</p>
            )}
          </div>
        </div>
      </div>
